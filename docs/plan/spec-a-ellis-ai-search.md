...
> **⚠ 방향 전환 (2026-07-27): 닷비즈 내장 삭제 → 고객사 Claude 플러그인.**
> ELLIS MCP를 닷비즈(Ohmy Partners) 포털 안에 신규 메뉴로 넣지 않는다. 대신 **고객사가 자사 Claude에
> 플러그인으로 붙여** 쓰는 방식으로 전환. 닷비즈 프로토타입의 AI 요금 검색 화면·메뉴는 삭제됨(2026-07-27).
> 아래 설계(자연어 파싱·Search Gateway·요금 정규화 등)는 **플러그인 형태로 재사용**되며, 전달 대상이
> '닷비즈 포털'에서 '고객사 Claude 환경'으로 바뀐다. 이 관점에서 §들을 재해석할 것.
>
> **작성일**: 2026-07-16 · **트랙**: A (AI 신기능) · **상태**: 설계 자산 유지 · 전달 형태 = 고객사 Claude 플러그인
...
| NLU | [llm/nlu-design.md](../llm/nlu-design.md) | 자연어→조건 변환 규칙 (과거날짜 금지·연도 가정 명시·**룸타입 키워드 인식**(요금제 필터용) 포함) |
